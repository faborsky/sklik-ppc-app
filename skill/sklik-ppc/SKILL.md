...
- **Nákupy / feed (Seznam Nákupy, ex-Zboží.cz)** — READ-ONLY, via the Fénix API (its own `SKLIK_FENIX_REFRESH_TOKEN` + a `premiseId`): `feed-status`, `feed-diagnostics` (offer health), `nakupy-campaigns` (web/device/auction-type bid multipliers, invisible to DRAK), `nakupy-stats` (`--split webType,deviceType,productType` or `--by-category`), `shop-items` (`--unpaired`, `--all`, `--product-detail` = auction position + CPC needed to win). Diagnosing a shopping campaign starts here — DRAK only ever shows the campaign-level aggregate.
  - Money is in CZK (not haléře) and a bid multiplier is `100 % = no change` (DRAK's `--device-bids` uses `0 = no change`) — never copy a multiplier between the two without converting. Web and auction-type multipliers can only be changed in the Sklik web UI; device ones write via `campaign-update --device-bids`.
...
- **Work in modest, sequential batches.** Use the `*-batch` commands instead of looping single creates, but keep each **write** batch within the method's `batchCallLimits` (typically **≤100 items** for create/update/remove). Over the cap you get status `413` — split the payload yourself; writes have no auto-chunking. Run batches one after another, not in parallel.
- **Reads are complete — listings and reports page themselves** (since v1.9.0). One API call returns at most 5000 rows (`statsDataLimit`), so the CLI walks offsets until it has everything: `campaigns`, `groups`, `keywords`, `negatives`, `ads`, `banners` and every `*-stats` return the WHOLE account, not a first page. Two consequences: a big account costs several requests per listing (the request budget handles it, but don't loop listings needlessly), and **numbers from these commands can be trusted as totals** — older versions silently capped `campaigns` at 100 and `ads`/`groups`/`banners` at 500 rows.
