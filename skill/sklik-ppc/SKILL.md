...
- **Campaigns**: `campaigns`, `campaign-create`, `campaign-update`, `campaign-remove`, `campaign-restore` (undelete), `campaign-stats`, `campaign-targeting`
  - Targeting flags on create/update: `--regions` (CSV region IDs), `--device-bids desktop:mobile:tablet:other` (%), `--schedule-json` (update only), `--ad-selection {weighted,random,cpa,cos}` (ad rotation — `random` = even split for a clean creative A/B test); `campaigns` reads the current rotation back in the `Rotation` column
